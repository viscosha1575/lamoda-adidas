...
export default function RafflePage() {
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      ...
      try {
        ...
        if (!cancelled) {
          setError(requestError.message || "Не удалось загрузить розыгрыш");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  async function handleWinner(playerId) {
    ...
    try {
      ...
      setSuccessMessage(`Игроку выдан codeId ${nextPlayer?.codeId ?? ""}`.trim());
    } catch (requestError) {
      setError(requestError.message || "Не удалось отметить победителя");
    } finally {
  ...
  async function handleFinishRaffle() {
    const confirmed = window.confirm("Завершить розыгрыш и отметить всех оставшихся участников как проигравших?");
    ...
    try {
      ...
      setSuccessMessage(`Розыгрыш завершен. Проигравшими отмечено: ${updatedCount}.`);
    } catch (requestError) {
      setError(requestError.message || "Не удалось завершить розыгрыш");
    } finally {
  ...
  async function handleExport() {
    ...
    try {
      ...
      downloadWorkbook(workbook, "raffle-winners.xlsx");
            ...
            >
              <InputGroup flex={{ base: "1 1 100%", lg: "1.5 1 0" }} minW="0">
                ...
                <Input
                  ...
                  placeholder="Поиск по Telegram ID или Telegram username"
                  ps="44px"
                  value={search}
              ...
              <Table variant="simple">
                <Thead>
                  <Tr>
                    ...
                    <Th color={textColorSecondary}>Результат</Th>
                    ...
                    <Tr>
                      <Td borderColor={borderColor} colSpan={6}>
                        <Text color={textColorSecondary} fontSize="sm" py="12px" textAlign="center">
                          Участники розыгрыша не найдены.
                        </Text>
                      </Td>
                    </Tr>
                  )}
